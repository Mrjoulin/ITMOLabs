...
class VideoTransformTrack(VideoStreamTrack):
    ...
    def get_average_people_num(self):
        return self.transform.get_average_people_num()
...
class StreamDetection:
    ...
    def detect(self, frame):
        ...
        if self.iteration % self.get_skip_frames_rate() > 0:
            logging.info('Skip frame in %.5f sec' % (time.time() - start_time))
            return None
        ...
        logging.info('Process frame in %.5f sec' % (time.time() - start_time))
    ...
    def log_average(self):
        logging.info(
            'Average time: %s (FPS %s); Average people: %s' % (
                self.get_average_time_render(),
                self.get_fps(),
                self.get_average_people_num(),
            )
        )
    ...
    def get_average_people_num(self):
        return np.round(np.mean(self.people_num), 3) if self.people_num else 0
