...
def show_functions_graph_by_dots(functions, dots, step):
    fig, ax = plt.subplots(1, 1, figsize=(10, 7), sharex=True, sharey=True, tight_layout=True)

    by_y = functions.pop("by_y") if "by_y" in functions else False

    bounds = [dots[0][0] - 0.1, dots[-1][0] + 0.1]
    x = np.linspace(bounds[0], bounds[1], max(len(dots) * 2, 100))
    ...
    # Show only dots with given step
    step_x = int(step / (dots[1, 0] - dots[0, 0]))
    step_dots = dots[::step_x]
    ax.scatter(step_dots[:, 0], step_dots[:, 1], c='r', label='Points')
