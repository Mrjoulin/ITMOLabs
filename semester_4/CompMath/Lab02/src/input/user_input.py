def read_from_file(is_system):
    ...
    while filename is None:
        ...
        try:
            ...
            if len(bounds) != 2 or (not is_system and bounds[0] == bounds[1]):
...
def get_approximations():
    ...
    while approx is None:
        print("Input X, Y approximations separated by a space:", end=" ")

        try:
            approx = list(map(float, input().split()))
