...
import entities.validators.annotations.Author
 ...
 *
 * @param collectionManager Is needed to generate new object with [UniqueId] annotation
 * @param author User object, is needed to set field with [Author] annotation
 *
...
class CreateEntityFromMap(
    ...
    private fun generateField(field: Field): Any? {
        ...
        if (field.isAnnotationPresent(UniqueId::class.java))
            return collectionManager.getDBWorker().generateId()
        else if (field.isAnnotationPresent(CurrentDate::class.java))
            return Date(System.currentTimeMillis())
        else if (field.isAnnotationPresent(Author::class.java))
