...
private var currentInput: InputStreamReader? = null
 ...
 * @param text Message, to show for user to get need input (if input not from file)
 * @return Split line witch received from method splitInput
 *
 * @author Matthew I.
 */
@Throws(IOException::class)
fun getInput(input: InputStreamReader, text: String = "") : ArrayList<String>? {
    if (input != currentInput) {
        currentInput = input
        reader = BufferedReader(input)
    }

    if (input.javaClass != FileReader::class.java) print(text)

    val line = reader?.readLine() ?: throw IOException("End of file!")
